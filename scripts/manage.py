...
@cli.command("remove-redundant-images")
@click.option('--dry-run', is_flag=True, help='Show what would be updated without making changes')
def remove_redundant_images(dry_run):
    """Remove redundant inline images from posts that now have featured images."""
    console.print("🧹 Checking for redundant inline images in posts with featured images...")

    # Find all markdown files
    md_files = list(Path("content/posts").glob("*.md")) + list(Path("content/posts").glob("*.markdown"))
    console.print(f"🔍 Found {len(md_files)} blog posts to analyze...")
    ...
    for post_file in sorted(md_files):
        try:
            ...
            if 'cover' not in post.metadata or 'image' not in post.metadata['cover']:
                console.print(f"⏭️  [dim]{post_file.name}[/dim] - No featured image")
                continue

            cover_image = post.metadata['cover']['image']

            # Find first image in content
            image_url, alt_text, image_position = find_first_image_in_content(post.content)

            if not image_url:
                console.print(f"🚫 [dim]{post_file.name}[/dim] - No inline images")
                continue

            # Check if the first inline image matches the featured image
            if image_url == cover_image:
                # Remove the redundant image from content
                lines = post.content.split('\n')

                # Find and remove the image line
                new_lines = []
                image_removed = False

                for line in lines:
                    # Match markdown image syntax and check if URL matches
                    if re.match(r'!\[.*?\]\(.*?\)', line) and image_url in line and not image_removed:
                        console.print(f"✅ [green]{post_file.name}[/green] - {'Would remove' if dry_run else 'Removed'} redundant image: {image_url[:60]}...")
                        image_removed = True
                        updated_count += 1
                        continue  # Skip this line (remove the image)
                    new_lines.append(line)

                if image_removed and not dry_run:
                ...
                console.print(f"🔄 [dim]{post_file.name}[/dim] - Featured image differs from first inline image")
    ...
    console.print(f"\n📊 [bold]Summary:[/bold]")
    console.print(f"  • Processed: {processed_count} posts")
    console.print(f"  • {'Would clean up' if dry_run else 'Cleaned up'}: {updated_count} posts")
    console.print(f"  • Skipped: {processed_count - updated_count} posts")
